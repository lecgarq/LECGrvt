...
| Rebuilt, redeployed, restarted Revit — behaviour unchanged | Revit is running a different `LECG.dll`. Two manifests can coexist (`%APPDATA%\Autodesk\Revit\Addins\2026\LECG.addin` and `C:\ProgramData\Autodesk\Revit\Addins\2026\LECG.addin`); the journal logs `Duplicate addins:` and, per ribbon button, the `assembly:` path it bound. Compare that file's timestamp with your build. Seen 2026-09-04: both manifests present, Revit loaded the `%APPDATA%` copy while the build deployed to ProgramData |
