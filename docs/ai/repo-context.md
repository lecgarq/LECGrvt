...
- **Deployment:** `DeployToRevit` MSBuild target copies output DLL/PDB/deps.json to `%APPDATA%\Autodesk\Revit\Addins\2026\LECG` after EVERY build unless `-p:SkipRevitDeploy=true` (`LECG.csproj:61-72`). CI sets `SkipRevitDeploy=true` automatically when `GITHUB_ACTIONS`/`CI` is set (`LECG.csproj:13`).
- **`.addin` manifest:** NOT build-generated and NOT deployed by `DeployToRevit` (which copies only dll/pdb/deps.json). The repo tracks only a template: `docs/deployment/LECG.addin.template`. The live manifest is manually installed per-user at `%APPDATA%\Autodesk\Revit\Addins\2026\LECG.addin` (`Type="Application"`, `FullClassName=LECG.App`, `Assembly=LECG\LECG.dll` relative, `<AddInId>` matching the template; journal-verified 2026-09-04). History: until 2026-09-04 a machine-wide copy under `C:\ProgramData\...` (with `<ClientId>`) was the documented location; when a per-user copy appeared alongside it Revit logged `Duplicate addins:` and loaded the per-user one, so the ProgramData copy was retired and `DeployToRevit` now targets `%APPDATA%`. Never keep both. Do not create/modify `.addin` files unless the user explicitly asks.
...
- [Observed] (scope: command — Warnings isolate path only) `View.IsolateElementsTemporary` requires an open transaction. Exercised in live Revit 2026 on 2026-07-27 against an open document with no ambient transaction: the bare call threw `Autodesk.Revit.Exceptions.ModificationOutsideTransactionException`; wrapped in a `Transaction` it succeeded, and `DisableTemporaryViewMode(TemporaryHideIsolate)` restored the view.
  - Evidence: `mcp-server-for-revit` probes 2026-07-27 (target `RD_Shower Enclosurs Showroom_R2020_M_V4`, `IsModifiable=False`); fix in `src/Services/Health/WarningsService.cs:77-91` | Why it matters: "temporary"/"view-only" Revit operations are still model modifications; and an MCP probe cannot answer transaction questions because the harness holds its own transaction | Preserve by: test the no-transaction path against a document whose `IsModifiable` is `False`, never against the MCP's active document.
  - Scope limit: this covers the isolate **API call** only. The Warnings ribbon button, dialog rendering, bindings, and select/show actions remain unvalidated in Revit — the deployed DLL still predates the feature.

### Known Concerns and Pitfalls
- [Concern] (scope: repo-wide) Plain `dotnet build` DEPLOYS: the `DeployToRevit` target copies DLLs into `%APPDATA%\Autodesk\Revit\Addins\2026\LECG` after every build unless `-p:SkipRevitDeploy=true`. The live per-user `LECG.addin` manifest points Revit at that exact folder, so a stray build overwrites the running add-in.
...
### Open Questions
- [Resolved 2026-09-04] Manifest GUID element: the live per-user manifest uses `<AddInId>`, same as the template. The `<ClientId>` variant existed only in the retired ProgramData copy. (Earlier resolution 2026-07-04 had pointed at the ProgramData install; superseded by the per-user move.)
