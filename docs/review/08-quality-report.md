...
- **Versioning**: Centralized in `Directory.Build.props`.
- **Hosted CI Gate**: `core-tests` required, coverage threshold enforced (`COVERAGE_MIN=90`).
- **Plugin CI Gate**: `plugin-build` workflow defined for self-hosted `revit2026` runner.
- **Release Automation**: Tag-based `release.yml` creates GitHub Releases with autogenerated notes.
- **Operational Runbook**: `docs/review/10-release-checklist.md`.
