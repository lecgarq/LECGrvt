...
**Success Criteria** (what must be TRUE):
  ...
  2. Outer boundary loops are counterclockwise and void loops are clockwise â€” `Floor.Create` and `Toposolid.Create` accept the output without `ArgumentException`
  3. `ISlabService.GetEditorVertexPositions(element)` returns a safe `List<XYZ>` snapshot before any modification â€” iterating the snapshot while modifying the editor does not throw
...
**Success Criteria** (what must be TRUE):
  ...
  2. Each converted element has the same boundary CurveLoops and the same number of interior shape points as its source â€” no points are lost or displaced
...
**Success Criteria** (what must be TRUE):
  ...
  2. The system identifies vertices at edges or transition zones whose Z values deviate from the surrounding surface and flags them â€” only Interior-type vertices are candidates for deletion; Edge vertices are corrected via `ModifySubElement`
  3. After repair, flagged vertices match the Z interpolation of the surrounding surface â€” the element renders without triangulation spikes
...
**Success Criteria** (what must be TRUE):
  1. User opens Split Boundaries, sees a list of selected elements with boundary counts, and elements with only one boundary are reported as not needing a split â€” no silent skips
  ...
  3. The Toposolid path uses the native `Toposolid.Split` API and the Floor path recreates one element per loop â€” both paths preserve type, level, and height offset on each result
...
**Success Criteria** (what must be TRUE):
  1. User opens the command, sees a checkbox list of element types with element counts, selects which types to export, defines an output folder, and starts export â€” no files are created before the user confirms
  ...
  3. Types that contain no geometry produce no output file â€” empty files are not created
...
**Execution Order:** P10 â†’ P11 â†’ P12 (can follow P11 or run after P10) â†’ P13 â†’ P14
...
| 15. Fix Geometry Operations | v2.0 | 2/2 | ✅ Completed | 2026-03-20 |

### Phase 15: Fix Geometry Operations
**Goal**: Repair and stabilize core geometry conversion and repair tools (Fix Points, Conversion Pair, Split Boundaries)
**Depends on**: Phase 11, 12, 13
**Requirements**: FIXP-01, CONV-01, SPLT-01
**Success Criteria**:
  1. Fix Points correctly repairs triangulations without creating gaps.
  2. Floor <-> Toposolid conversion preserves all shape points and absolute elevations.
  3. Split Boundaries correctly handles complex island scenarios for Toposolids.
