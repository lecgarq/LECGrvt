...
if (!/^\d{8}T\d{6}$/.test(runName ?? '')) throw new Error('Use export-project-native-read-evidence.mjs <run-name>.');
const run = join(evidence, 'project-native-read-runs', runName, 'ProjectNativeRead');
const output = join(root, 'RevitCopilot/Agent/Knowledge/revit2026-native-project-reads.json');
...
const manifest = read(join(evidence, 'project-native-read-membership-manifest.json'));
...
for (const { value } of receipts) {
  require(disciplines.has(value.model) && value.status === 'read-only-native-validation'
    ...
    && value.read_operation_count === 49 && value.reads.length === 49
    && new Set(value.reads.map(row => row.operation)).size === 49
...
require(operations.length === 49 && maps.every(map => operations.every(operation => map.has(operation))),
  'Native-read operation sets differ across models.');
for (const operation of ['curve_join_neighbors', 'assigned_electrical_systems', 'spatial_contains_point', 'mep_connectors',
  'external_files_list', 'panel_host', 'stairs_associated_railings', 'group_attached_detail_types',
  'base_points', 'face_split_boundaries', 'group_members', 'family_subcomponents',
  'assembly_members', 'mep_system_members'])
...
const pack = {
  ...
  scope: 'Production MCP native-read path executed in four named CASA EUCALIPTO disposable copies; no returned model values were retained and outcomes are context evidence, not universal applicability.',
