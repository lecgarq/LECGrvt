...
const manifest = read(join(evidence, 'project-native-read-geometry-manifest.json'));
...
for (const { value } of receipts) {
  require(disciplines.has(value.model) && value.status === 'read-only-native-validation'
    ...
    && value.read_operation_count === 45 && value.reads.length === 45
    && new Set(value.reads.map(row => row.operation)).size === 45
...
require(operations.length === 45 && maps.every(map => operations.every(operation => map.has(operation))),
...
for (const operation of ['curve_join_neighbors', 'assigned_electrical_systems', 'spatial_contains_point', 'mep_connectors',
  ...
  'base_points', 'face_split_boundaries'])
