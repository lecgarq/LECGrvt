...
$target = Join-Path $evidence 'project-native-read-associations-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-read associations manifest already frozen.' }
$sourcePath = Join-Path $evidence 'project-native-read-coordination-manifest.json'
...
$manifest = [ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-read-associations-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;read_operation_count=43
    test_class='ProjectNativeReadBatch';campaign_prefix='project-native-read-associations'
    source_manifests=@([ordered]@{path='project-native-read-coordination-manifest.json';sha256=(Get-FileHash $sourcePath).Hash})
