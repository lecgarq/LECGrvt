...
$target = Join-Path $evidence 'project-native-read-geometry-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-read geometry manifest already frozen.' }
$sourcePath = Join-Path $evidence 'project-native-read-associations-manifest.json'
...
$manifest = [ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-read-geometry-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;read_operation_count=45
    test_class='ProjectNativeReadBatch';campaign_prefix='project-native-read-geometry'
    source_manifests=@([ordered]@{path='project-native-read-associations-manifest.json';sha256=(Get-FileHash $sourcePath).Hash})
