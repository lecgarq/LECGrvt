...
$target = Join-Path $evidence 'project-native-read-relations-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-read relations manifest already frozen.' }
$sourcePath = Join-Path $evidence 'project-native-read-expansion-manifest.json'
...
$manifest = [ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-read-relations-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;read_operation_count=39
    test_class='ProjectNativeReadBatch';campaign_prefix='project-native-read-relations'
    source_manifests=@([ordered]@{path='project-native-read-expansion-manifest.json';sha256=(Get-FileHash $sourcePath).Hash})
