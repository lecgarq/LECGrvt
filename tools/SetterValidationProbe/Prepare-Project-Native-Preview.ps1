...
$target = Join-Path $evidence 'project-native-preview-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-preview manifest already frozen.' }
...
$manifest = [ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-preview-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;preview_operation_count=17
    test_class='ProjectNativePreviewBatch';campaign_prefix='project-native-preview'
...
"Frozen: $($manifest.preview_operation_count) native previews across $($manifest.models.Count) models."
