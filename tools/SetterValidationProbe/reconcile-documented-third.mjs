...
function reconcile(runName, trxName) {
  ...
  require(/^documentedthird-\d{8}T\d{6}\.trx$/.test(trxName ?? ''), 'Provide the batch TRX.');
  const manifestPath = join(evidence, 'documented-third-manifest.json');
  const manifest = read(manifestPath);
  const run = join(evidence, 'documented-third-runs', runName);
  ...
  require(hash(join(evidence, 'documented-third-preregistration.md')) === manifest.preregistration_sha256,
  ...
  require(/\btotal="6"/.test(counters) && /\bpassed="6"/.test(counters)
    && /\bfailed="0"/.test(counters) && trx.includes('DocumentedThirdBatch'),
  'TRX does not establish six completed cases.');
  const priorRun = ledger.campaign.match(/^changed-value-documented-next-retry-(\d{8}T\d{6})/)?.[1];
  require(priorRun, 'Ledger does not identify the prior reconciliation.');
  const previousReportPath = join(evidence, 'documented-next-retry-runs', priorRun, 'ledger-reconciliation.json');
  ...
  const promotionModes = new Map([
    ['api.set:Autodesk.Revit.DB.Architecture.ContinuousRailType.EndOrTopTermination', 'within'],
    ['api.set:Autodesk.Revit.DB.Architecture.ContinuousRailType.StartOrBottomTermination', 'within'],
    ['api.set:Autodesk.Revit.DB.FamilyInstance.StructuralUsage', 'project']
  ]);
  ...
  for (const plan of manifest.cases) {
    ...
    const mode = promotionModes.get(plan.operation);
    require(validations.length === (mode ? 1 : 0), `Unexpected validation count: ${plan.operation}`);
    if (mode) {
      const index = proposed.entries.findIndex(entry => entry.operation === plan.operation);
      const before = proposed.entries[index];
      proposed.entries[index] = mode === 'within'
        ? promoteWithinCorpus(before, validations[0].value)
        : appendProjectAttempt(before, validations[0].value);
      updates.push({ mode, before, after: proposed.entries[index], receipt: reference(validations[0].path) });
    }
  ...
  require(setters.length === 805 && numerator - originalNumerator === 3,
    'Denominator or expected three validations changed.');
  ...
  const report = { run: runName, original_ledger_sha256: manifest.ledger_sha256,
    ...
    original_numerator: originalNumerator, proposed_numerator: numerator, new_validations: 3,
    denominator: 805, counts_by_state: countsByState, updates, outcomes };
  const reportText = JSON.stringify(report, null, 2) + '\n';
  proposed.campaign = 'changed-value-documented-third-' + runName;
  proposed.source_sha256 = createHash('sha256').update(reportText).digest('hex').toUpperCase();
  proposed.scope += ' 3 direct-API validations: both continuous-rail termination setters in the Autodesk corpus and FamilyInstance.StructuralUsage in the CASA EUCALIPTO structural project copy. ';
  const results = { report: 'documented-third-6', new_validations: 3, entries: outcomes };
  for (const [name, value] of [['ledger-proposal.json', proposed], ['ledger-reconciliation.json', report],
    ['documented-third-results.json', results]])
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    require(process.argv.length === 4, 'Use reconcile-documented-third.mjs <run-name> <trx-filename>.');
