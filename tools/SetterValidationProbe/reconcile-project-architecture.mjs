...
function reconcile(runName, trxName) {
  ...
  require(/^projectarchitecture-\d{8}T\d{6}\.trx$/.test(trxName ?? ''), 'Provide the batch TRX.');
  const manifestPath = join(evidence, 'project-architecture-manifest.json');
  const manifest = read(manifestPath);
  const run = join(evidence, 'project-architecture-runs', runName);
  ...
  require(hash(join(evidence, 'project-architecture-preregistration.md')) === manifest.preregistration_sha256,
  ...
  require(manifest.models.length === 1 && manifest.models[0].discipline === 'architecture',
    'Expected one architecture fixture.');
  ...
  const source = resolve(fixtureRoot, manifest.models[0].relative_path);
  require(!relative(resolve(fixtureRoot), source).split(/[\\/]/).includes('..')
    && hash(source) === manifest.models[0].sha256, 'Architecture source changed.');
  require(manifest.cases.length === 4 && manifest.baseline_entries.length === 4
  ...
  completedTrx(trxPath, 4, manifest.test_class);
  const planned = evidenceForPlan(run, manifest, 4);
  ...
  for (const item of planned) {
    ...
    require(index >= 0 && total(proposed.entries[index]) === 12,
      `Unexpected baseline model total: ${item.plan.operation}`);
  ...
  const report = { run: runName, original_ledger_sha256: manifest.ledger_sha256,
    ...
    provenance: reference(provenancePath), trx: reference(trxPath), source_sha256: hash(source),
  ...
  const results = { report: 'project-architecture-4', new_validations: newValidations, entries: outcomes };
  for (const [name, value] of [['ledger-proposal.json', proposed], ['ledger-reconciliation.json', report],
    ['project-architecture-results.json', results]])
...
try {
  require(process.argv.length === 4, 'Use reconcile-project-architecture.mjs <run-name> <trx-filename>.');
