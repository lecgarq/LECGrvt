...
$target = Join-Path $evidence 'project-native-read-coordination-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-read coordination manifest already frozen.' }
$sourcePath = Join-Path $evidence 'project-native-read-relations-manifest.json'
...
$manifest = [ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-read-coordination-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;read_operation_count=41
    test_class='ProjectNativeReadBatch';campaign_prefix='project-native-read-coordination'
    source_manifests=@([ordered]@{path='project-native-read-relations-manifest.json';sha256=(Get-FileHash $sourcePath).Hash})
