...
$target = Join-Path $evidence 'project-native-read-manifest.json'
if (Test-Path -LiteralPath $target) { throw 'Project native-read manifest already frozen.' }
$sourcePath = Join-Path $evidence 'project-accessor-read-manifest.json'
...
$manifest=[ordered]@{
    ...
    preregistration_sha256=(Get-FileHash (Join-Path $evidence 'project-native-read-preregistration.md')).Hash
    snapshot_amendment_sha256=$source.snapshot_amendment_sha256
    copilot_assembly_sha256=(Get-FileHash $copilotPath).Hash;read_operation_count=35
    test_class='ProjectNativeReadBatch';campaign_prefix='project-native-read'
    source_manifests=@([ordered]@{path='project-accessor-read-manifest.json';sha256=(Get-FileHash $sourcePath).Hash})
